...
abstract class BaseReportCommentsQuery extends ModelCriteria
{
     ...
     * @param   ReportCommentsQuery|Criteria $criteria Optional Criteria to build the query from
     *
    ...
    public static function create($modelAlias = null, $criteria = null)
    {
        ...
        $query = new ReportCommentsQuery(null, null, $modelAlias);
     ...
     *
     * @return   ReportComments|ReportComments[]|mixed the result, formatted by the current formatter
     */
    public function findPk($key, $con = null)
    {
        ...
        if ((null !== ($obj = ReportCommentsPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
     ...
     *
     * @return                 ReportComments A model object, or null if the key is not found
    ...
    {
        $sql = 'SELECT `id`, `index`, `comments`, `report_id`, `volunteer_id` FROM `report_comments` WHERE `id` = :p0';
        try {
        ...
        if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            $obj = new ReportComments();
            $obj->hydrate($row);
            ReportCommentsPeer::addInstanceToPool($obj, (string) $key);
        }
        $stmt->closeCursor();
     ...
     * @param     PropelPDO $con an optional connection object
     *
     * @return PropelObjectCollection|ReportComments[]|mixed the list of results, formatted by the current formatter
     */
    ...
    public function filterById($id = null, $comparison = null)
    {
        if (is_array($id)) {
            $useMinMax = false;
            if (isset($id['min'])) {
                $this->addUsingAlias(ReportCommentsPeer::ID, $id['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($id['max'])) {
                $this->addUsingAlias(ReportCommentsPeer::ID, $id['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
