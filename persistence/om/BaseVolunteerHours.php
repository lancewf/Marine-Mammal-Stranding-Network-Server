  ...
  const PEER = 'VolunteerHoursPeer';
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 8; // 8 = VolunteerHoursPeer::NUM_COLUMNS - VolunteerHoursPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(VolunteerHoursPeer::ID) ) {
					...
					$affectedRows += VolunteerHoursPeer::doUpdate($this, $con);
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setDayOfMonth($this->day_of_month);
