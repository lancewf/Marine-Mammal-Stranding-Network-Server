...
abstract class BaseBlogEntryCommentsQuery extends ModelCriteria
{
     ...
     * @param   BlogEntryCommentsQuery|Criteria $criteria Optional Criteria to build the query from
     *
    ...
    public static function create($modelAlias = null, $criteria = null)
    {
        ...
        $query = new BlogEntryCommentsQuery(null, null, $modelAlias);
     ...
     *
     * @return   BlogEntryComments|BlogEntryComments[]|mixed the result, formatted by the current formatter
     */
    public function findPk($key, $con = null)
    {
        ...
        if ((null !== ($obj = BlogEntryCommentsPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
     ...
     *
     * @return                 BlogEntryComments A model object, or null if the key is not found
    ...
    {
        $sql = 'SELECT `id`, `index`, `comments`, `blog_entry_id`, `volunteer_id` FROM `blog_entry_comments` WHERE `id` = :p0';
        try {
        ...
        if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            $obj = new BlogEntryComments();
            $obj->hydrate($row);
            BlogEntryCommentsPeer::addInstanceToPool($obj, (string) $key);
        }
        $stmt->closeCursor();
     ...
     * @param     PropelPDO $con an optional connection object
     *
     * @return PropelObjectCollection|BlogEntryComments[]|mixed the list of results, formatted by the current formatter
     */
    ...
    public function filterById($id = null, $comparison = null)
    {
        if (is_array($id)) {
            $useMinMax = false;
            if (isset($id['min'])) {
                $this->addUsingAlias(BlogEntryCommentsPeer::ID, $id['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($id['max'])) {
                $this->addUsingAlias(BlogEntryCommentsPeer::ID, $id['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
     ...
     * $query->filterByBlogEntryId(array('max' => 12)); // WHERE blog_entry_id <= 12
     * </code>
     *
     ...
     *
     * @param   BlogEntry|PropelObjectCollection $blogEntry The related object(s) to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 BlogEntryCommentsQuery The current query, for fluid interface
    ...
    public function filterByBlogEntry($blogEntry, $comparison = null)
    {
        ...
        } elseif ($blogEntry instanceof PropelObjectCollection) {
            if (null === $comparison) {
                $comparison = Criteria::IN;
            }
